...
public class MunicipioDAO implements BaseDAO<Municipio> {
    ...
    public Municipio readById(Long id, Connection conn) throws Exception {
        Municipio e = null;
        String sql = "SELECT municipio.*, uf.id as uf_id, uf.nome as uf_nome, uf.sigla as uf_sigla FROM municipio LEFT JOIN uf ON municipio.uf_fk = uf.id WHERE municipio.id=?";
        PreparedStatement ps = conn.prepareStatement(sql);
        ps.setLong(1, id);
        ResultSet rs = ps.executeQuery();
        if (rs.next()) {
    ...
    public List<Municipio> readByCriteria(Map<String, Object> criteria, Connection conn) throws Exception {
        ...
        String sql = "SELECT municipio.*, uf.id as uf_id, uf.nome as uf_nome, uf.sigla as uf_sigla FROM municipio LEFT JOIN uf ON municipio.uf_fk = uf.id WHERE 1=1";
